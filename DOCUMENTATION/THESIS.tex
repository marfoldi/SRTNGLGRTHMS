...
\hiddensubsubsection{Működés}
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/bubble.jpg}
...
Lényege, hogy a soron következő elemet - a második elemtől kezdve - egy ideiglenes változóba mentjük, és a rendezett tömbrész elemeit jobbra csúsztatjuk mindaddig, amíg a kiválasztott érték a helyére nem kerül. Ezt n-1 alkalommal ismételve megkapjuk a rendezett tömböt.
 \hiddensubsubsection{Példa}
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/insertion.jpg}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		$2^p3^q$ alakú számok & 1, 2, 3, 4, 6 $\ldots$  &               $\Theta(n \log^2 n)$                     & Pratt, 1971     \\ \hline
	\end{tabular}
	\caption{Lépésköz megválasztásának módszerei}
\end{table}
 \hiddensubsubsection{Példa}
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/shell.jpg}
...
\begin{description}
	...
	\item[$\bullet$] Válasszunk egy vezérelemet (például a legjobb oldalibb elem).
	\item[$\bullet$] Osszuk két részre a rendezendő részt, mozgassuk a vezérelemtől kisebb- vagy egyenlő elemeket a tömb elejére, míg a nagyobbakat végébe \cite{Ronyai}.
	\item[$\bullet$] Rekurzívan ismételjük meg az előbbi lépéseket a résztömbökön.
\end{description}
Gyakorlatilag szétválasztást eszközölünk, melynek eredményeképp a kiválasztott elemtől kisebb vagy egyenlő értékek tőle balra, a nagyobbak pedig jobbra helyezkednek el. Az egyszerűbb megérthetőséget szem előtt tartva a program mindig a legjobb oldalibb elemet választja vezérelemnek.
 \hiddensubsubsection{Példa}
 Az eddigi példákkal megegyezően a bemenet: $42, 22, 10, 57$.\par
 Az első képen látható a már kiválasztott vezérelem. A negyedik ábráig csak összehasonlítások történnek, mivel minden elem kisebb, mint a kiválasztott érték. A negyedik képen a három elemű résztömbön végezzük el a rendezést, a vezérelem ismét a legjobb oldalibb érték. Az ötödik képen az első érték nagyobb mint a vezérelem, így ezt az elemet a hatodik képen megcseréljük a vezérelemmel. Ezt követően a középső két elem kerül összehasonlításra, mivel nincs szükség cserére így a rendezés befejeződött.\par
 \begin{figure}[H]
 	\centering
...
Az algoritmus a bemeneti elemekből kupacot épít, majd a legfelsőbb elemét felcseréli a kupac legjobb oldalibb levelével. Így a legnagyobb elem a helyére került, az többé már nem tartozik a kupachoz, a kupac mérete eggyel csökken. Ezt követően ellenőrzi a kupac tulajdonságokat. A gyökérelem sértheti a "szülő nagyobb mindkét gyerekénél" szabályt, így azt cserékkel addig süllyeszti, míg helyre nem áll a kupac tulajdonság. Eközben a kupac tetejére kerül a következő legnagyobb elem. Ezt az elemet ismét felcseréli a kupachoz tartozó legjobb oldalibb levéllel, az így helyére került elemet kizárja a kupacból, és ismét helyreállítja a kupac tulajdonságot. Ezen lépéseket addig ismétli, amíg a kupac egy eleművé nem válik. A rendezés implementálásánál a kupacot minden esetben egy tömb ábrázolja, mely a kupacot ábrázoló bináris fa szintfolytonos ábrázolásának tekinthető. A tömbön belül egyre zsugorodik a kupac, míg a tömb végén kezd kialakulni a rendezett sorozat.\par
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/heap.jpg}
 	\caption{Példa a kupacrendezésre}
 \end{figure}
\hiddensubsubsection{Műveletigény}
A kupacrendezés egyik további előnye, hogy míg a gyorsrendezésnek legrosszabb esetben a futási ideje $\mathcal{O}(n^2)$, addig itt a futási idő továbbra is $\mathcal{O}(n\cdot\log (n))$. Ennél fogva azon rendszereknél, ahol a négyzetes futási idő elfogadhatatlan inkább kupacrendezést alkalmaznak.
\hiddensubsubsection{Jelölések az állapotjelző felületen}
...
Az első menetben a rendezendő elemek első bitjét vizsgálja az algoritmus. A vizsgálat két mutatóval történik, melyek a tömb két végéről indulnak. A tömb elején addig halad a mutató, amíg a vizsgált elem első jegye nem 1, ezzel párhuzamosan a tömb végén olyan elemet keres a másik, melynek első jegye 0. Amennyiben talált ilyen elemeket, megcseréli őket. Ezt mindaddig folytatódik, amíg a két mutató nem találkozik. Ekkor kialakul két edény, az elsőben a 0-ás kezdőbittel rendelkező számok, míg a másodikban az 1-essel kezdődő elemek foglalnak helyet. Ezt követően a második bit kerül vizsgálatra az "aledényekben", az előzővel azonos módon. A rendezés befejeződött, ha minden számjegy szerinti vizsgálat megtörtént, vagy ha mindegyik, a futás alatt kialakult edény már csak egy elemet tartalmaz.
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/radixforward.jpg}
...
Az előző algoritmustól eltérően már nem helyben rendez, az eljárásnak két tömbre van szüksége. További különbség, hogy a kisebb helyiértéktől a nagyobb felé halad a vizsgálat.\par Amennyiben az aktuálisan vizsgált bit értéke 0, akkor a "második" tömb elejére, ellenkező esetben a tömb végére töltjük át az aktuális bináris számot. Ezáltal minden egyes menetben két edény keletkezik: egy, melyben található számok aktuális bitje 0, s egy másik, melyek vizsgált jegye 1-es. Ezt követően a 0-ás edényt az elejéről olvasva feltöltjük újra az eredeti tömböt az előbb ismertetett módon, majd a 1-es edényt elemeit az utolsó elemtől visszafelé haladva töltjük át az értékeket. Amennyiben a rendezendő bináris számok $d$ hosszúak, úgy $d+1$ áttöltést követően rendezett tömböt kapunk.
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[width=\textwidth]{pics/radixbackward.jpg}
...
Az osztályban található metódusok kezelik a menüsor elemein való kattintást (\textbf{handleAbout()} és \textbf{handleExit()} függvények). Továbbá a gombokon történő kattintás következtében betöltik a bemenet megadására lehetőséget adó panelekhez tartozó \emph{FXML} fájlokat.
...
A harmadik gombra kattintás követően keletkezik belőle objektum. Beállítja a fájlkiterjesztés szűrőt és megjeleníti a fájl tallózót. Továbbá itt történik meg a fájl tartalmának az ellenőrzése. Az előző két osztállyal azonosan végzi el a bemenő adatok átadását, valamint a főpanelek betöltését.
\hiddenparagraph{BaseController}
A főpanelekhez tartozó kontroller osztály. Figyeli, hogy 100 elemnél hosszabb-e a bemenet, amennyiben igen eltünteti az \textbf{Megfigyelés} panelt. Valamint metódusai kezelik az menüsor egy elemén való kattintást.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|l|}
		\hline
		\textbf{Algoritmus} & \multicolumn{1}{c|}{\textbf{Elvárt értékek}}                             & \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Program által\\ számított értékek\end{tabular}}} \\ \hline
		...
		Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}31 összehasonlítás\\ 19 mozgatás\end{tabular} & \begin{tabular}[c]{@{}l@{}}31 összehasonlítás\\ 19 mozgatás\end{tabular}                                \\ \hline
	\end{tabular}
	\caption{Öt elem rendezésekor a műveletszámok}
\end{table}
\paragraph{sixteenConcretElement()}
A $[307, 42, 200, 307, 65, 2, 89, 120, 189, 420, 55, 12, 7,\newline 99, 510, 120]$
 tizenhat elemű tömbön futtatja le a rendezéseket. Az előző példával megegyezően ellenőrzésre kerül a rendezés eredményének helyessége és a műveletszámok összege.
 \begin{table}[H]
 	\centering
 	\begin{tabular}{|c|l|l|}
 		\hline
 		...
 		Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}135 összehasonlítás\\ 75 mozgatás\end{tabular} & \begin{tabular}[c]{@{}l@{}}135 összehasonlítás\\ 75 mozgatás\end{tabular}                               \\ \hline
 	\end{tabular}
 	\caption{Tizenhat elemű bemenetre a műveletszámok}
 \end{table}
 \paragraph{tenSortedElement()}
 Tíz elemű, már eleve rendezett tömbön futtatja az eljárásokat. A bemeneti tömb: $[2, 42, 65, 89, 120, 189, 200, 291, 307, 420]$. Ellenőrzi a tömb tartalmát és a műveletszámokat. A lenti táblázat összefoglalja az program által számított- és az elvárt értékeket:
 \begin{table}[H]
 	\centering
 	\begin{tabular}{|c|l|l|}
 		\hline
 		...
 		Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}87 összehasonlítás\\ 51 mozgatás\end{tabular} & \begin{tabular}[c]{@{}l@{}}135 összehasonlítás\\ 75 mozgatás\end{tabular}                               \\ \hline
 	\end{tabular}
 	\caption{Tíz elemű, már eleve rendezett tömbön a műveletszámok}
 \end{table}
\paragraph{tenReverseSortedElement()}
Tíz csökkenő sorrendbe rendezett értéken indítja el az algoritmusokat. A bemeneti tömb: $[420, 307, 291, 200, 189, 120, 89, 65, 42, 2]$. Ellenőrzésre kerül a rendezett tömb tartalma, továbbá a műveletek számának összege. A következő táblázat összefoglalja az program által számított- és az elvárt értékeket:
 \begin{table}[H]
 	\centering
 	\begin{tabular}{|c|l|l|}
 		\hline
 		...
 		Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}87 összehasonlítás\\ 51 mozgatás\end{tabular} & \begin{tabular}[c]{@{}l@{}}31 összehasonlítás\\ 19 mozgatás\end{tabular}                                \\ \hline
 	\end{tabular}
 	\caption{Tíz csökkenőleg rendezett elemre a műveletszámok}
 \end{table}
\paragraph{tenAlmostSortedElement()}
A bemeneti tömb: $[1, 20, 56, 76, 55, 127, 160, 132,\newline 191, 243]$. Amint látható, már majdnem rendezett a sorozat, két érték nincs még a helyén ($55$ és $132$). Ellenőrzésre kerül a rendezések befejeztével a tömb tartalma, továbbá a műveletek számának összege. A következő táblázat összefoglalja az program futása során kapott- és az elvárt értékeket:
\begin{table}[H]
	\centering
	\begin{tabular}{|c|l|l|}
		\hline
		...
		Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}87 összehasonlítás\\ 51 mozgatás\end{tabular} & \begin{tabular}[c]{@{}l@{}}87 összehasonlítás\\ 51 mozgatás\end{tabular}                                \\ \hline
	\end{tabular}
	\caption{Tíz elemű, majdnem rendezett bemenetre a műveletszámok}
\end{table}
\paragraph{thousandRandomElement()}
Száz véletlen választott értéken hívja meg a rendezéseket. A műveletek száma nem kerül ellenőrzésre, csak az előbb említett módon a tömb tartalma. A következő táblázat egy teszt futtatásakor a műveletszámok eredményét összegzi:
\begin{table}[H]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|c|l|l|}
			\hline
			\textbf{Algoritmus} & \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Program által\\ számított értékek\end{tabular}}} & \multicolumn{1}{c|}{\textbf{Eredmény}}                                                                                                                                                                                          \\ \hline
			Buborékrendezés     & \begin{tabular}[c]{@{}l@{}}4950 összehasonlítás\\ 2285 csere\end{tabular}                               & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n^2) \approx 10^4 = 10000$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                                  \\ \hline
			Beszúró rendezés    & \begin{tabular}[c]{@{}l@{}}2546 összehasonlítás\\ 2452 mozgatás\end{tabular}                            & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n^2) \approx 10^4 = 10000$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                                  \\ \hline
			Shell rendezés      & \begin{tabular}[c]{@{}l@{}}1540 összehasonlítás\\ 226 mozgatás\end{tabular}                             & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n \log^2 (n)) \approx  2121$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                                 \\ \hline
			Gyorsrendezés       & \begin{tabular}[c]{@{}l@{}}672 összehasonlítás\\ 311 csere\end{tabular}                                 & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n\cdot\log(n)) \approx 664$ feletti, de a maximálisan\\ megengedett $\Theta(n^2) \approx 10^4 = 10000$ alatti az összehasonlítások száma.\end{tabular}                                                                               \\ \hline
			Kupacrendezés       & \begin{tabular}[c]{@{}l@{}}1018 összehasonlítás\\ 580 csere\end{tabular}                                & \begin{tabular}[c]{@{}l@{}}Az átlagosan- és maximálisan elvárt $\Theta(n\cdot\log(n)) \approx 664$ feletti\\ az összehasonlítások száma, ám ez betudható a konstans szorzónak.\end{tabular}                                                                         \\ \hline
			Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}1513 összehasonlítás\\ 820 mozgatás\end{tabular}                             & \begin{tabular}[c]{@{}l@{}}Mivel a bemenet hossza nem négyzetszám, így 128 elemre\\ van meghívva a rendezés. A maximális $\Theta(n\cdot\log(n)) \approx 896$ feletti\\ az összehasonlítások száma, ami a konstans szorzó miatt elfogadható.\end{tabular} \\ \hline
		\end{tabular}
	}
	\caption{Száz generált bemenetre egy futtatás eredménye}
\end{table}
\paragraph{millionRandomElement}
Az előző eljárással azonos, mindössze a tömb mérete különbözik. Ez feleltethető meg a "nagy" bemenetre történő ellenőrzésnek, mivel csak ezek a rendezések hívódnak meg száz elemnél hosszabb bemenet megadásakor. A teszt egy futtatásának az eredményét részletei az alábbi táblázat:
\begin{table}[H]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|c|l|l|}
			\hline
			\textbf{Algoritmus} & \multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Program által \\ számított értékek\end{tabular}}} & \multicolumn{1}{c|}{\textbf{Eredmény}}                                                                                                                                                                                                      \\ \hline
			Buborékrendezés     & \begin{tabular}[c]{@{}l@{}}499999500000 összehasonlítás\\ 250001268024 csere\end{tabular}                           & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n^2) \approx 10^{12} = 1000000000000$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                              \\ \hline
			Beszúró rendezés    & \begin{tabular}[c]{@{}l@{}}250002268014 összehasonlítás\\ 250001268024 mozgatás\end{tabular}                             & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n^2) \approx 10^{12} = 1000000000000$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                              \\ \hline
			Shell rendezés      & \begin{tabular}[c]{@{}l@{}}134479666 összehasonlítás\\ 11716324 mozgatás\end{tabular}                       & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n \log^2 (n)) \approx  4,8*10^{11} = 480453013918$\\ műveletszám alatt vannak az értékek.\end{tabular}                                                                                                                            \\ \hline
			Gyorsrendezés       & \begin{tabular}[c]{@{}l@{}}25543945 összehasonlítás\\ 12530736 csere\end{tabular}                        & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt $\Theta(n\cdot\log(n)) \approx 19931568$ feletti\\ az összehasonlítások száma, de a maximális $\Theta(n^2) \approx 10^{12} = 1000000000000$ alatti.\end{tabular}                                            \\ \hline
			Kupacrendezés       & \begin{tabular}[c]{@{}l@{}}36794678 összehasonlítás\\ 19048525 csere\end{tabular}                        & \begin{tabular}[c]{@{}l@{}}Az átlagosan elvárt, és a maximálisan megengedett $\Theta(n\cdot\log(n)) \approx 19931568$\\ feletti az összehasonlítások száma, ám ez betudható a konstans szorzónak.\end{tabular}                                                       \\ \hline
			Versenyrendezés     & \begin{tabular}[c]{@{}l@{}}41048535 összehasonlítás\\ 21048555 mozgatás\end{tabular}                        & \begin{tabular}[c]{@{}l@{}}Mivel a bemenet nem kettő hatvány, így $2^{20} = 1048576$ elemű a bemenet.\\ A maximális $\Theta(n\cdot\log(n)) \approx 1.99 * 10^7 = 20971520$ feletti\\ az összehasonlítások száma, ami a konstans szorzó miatt elfogadható.\end{tabular} \\ \hline
		\end{tabular}
	}
	\caption{Egymillió generált bemenetre a teszt futásának eredménye}
\end{table}
